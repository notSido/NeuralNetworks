...
### 1.2 The Beginning of The Computing Era
...
### 1.3 Transition to the era of modern neural networks
...
### 2.1 The Big (Generative AI) Bang
...
### 2.3 Text-to-text Models and the birth of the AI assistant
...
Please note that these embedding values are not accurate at all, and the values were adjusted to make them easier to see, a real dictionary of embedding values would be **massive** and **utterly impossible** to interpret visually. This is just a representation of what embedding values **might** look like displayed in three dimensions, which is the highest amount of dimensions we as humans can comprehend, math and LLMs however do not care about such lowly human limitations. Here's some facts to help put the ***unimaginable, behemothic, incomprehensible complexity*** of real token embeddings into perspective:

* BERT: The original BERT model ("BERT-Base") uses embeddings of dimension **768**. There are also larger versions like "BERT-Large" which use embeddings of dimension **1024**.
* GPT: The *smallest* GPT-2 model uses embeddings of dimension **768**, and this increases with larger versions of the model. GPT-3, for instance, uses embeddings with a dimensionality as high as **12288** in its largest configuration.

As previously mentioned, there would not even a point in trying to create a visually comprehensible graph of these dimensionalities.

This however is no problem, as we don't have to be able to see the embeddings. Again, math and LLMs do not care, to them, numbers are nothing more than numbers, and words are nothing more than a huge collection of vectors in ***thousands*** of dimensions.
...
# apply the sigmoid function to each value in the 'x' column
df['y'] = sigmoid(df['x'])
...
4   5     0.992475
```
As you can see, the `sigmoid` function has been applied to each value in the 'x' column of the dataframe,
resulting in a new column containing the product of the sigmoid function.

Alternatively, you can use the `pd.Series` method `apply()` to apply the sigmoid function to each value in the
dataframe:
```Python
# apply the sigmoid function to each value in the 'x' column
...
These models go through extensive fine-tuning and output filtering to prevent them from answering certain questions, like ones regarding unethical or illegal activities.
...
##### AI Assistants are spineless by design
...
##### LLMs do <u>not</u> use language like we do
It is important to note that LLMs are not sentient, meaning they <u>*do not understand*</u> any of the text you feed them as input or the text they return as the output, nor are they capable of understanding logic and reason. <a id="LLM-sentience"></a>

##### LLMs lie like Project Managers
